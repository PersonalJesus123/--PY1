...
def to_csv_file(OUTPUT_FILE, headers_list, data, delimiter=',', newline='\n'):
    with open(OUTPUT_FILE, 'w') as f:
        f.write(delimiter.join(headers_list) + newline)
        for swe in data:
            f.write(delimiter.join(swe) + newline)
    return to_csv_file(OUTPUT_FILE, headers_list, data, delimiter=',', newline='\n')
